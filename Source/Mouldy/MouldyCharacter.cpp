...
AMouldyCharacter::AMouldyCharacter()
{
...
void AMouldyCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{
		UE_LOG(LogMouldy, Error, TEXT("'%s' Failed to find an Enhanced Input component! This template is built to use the Enhanced Input system. If you intend to use the legacy system, then you will need to update this C++ file."), *GetNameSafe(this));
	}
}

void AMouldyCharacter::Move(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AMouldyCharacter::DoMove(float Right, float Forward)
{
	if (GetController() != nullptr)
	{
		...
		const FRotator YawRotation(0, 45, 0);
...
void AMouldyCharacter::DoJumpEnd()
{
